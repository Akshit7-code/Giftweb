...
const SignatureCollection = ({ products }) => {
  ...
  return (
    <section className="h-screen w-full bg-[#FAFAFA] overflow-hidden flex flex-col py-16">
      {/* Matched with ProductCards: 
         max-w-[1630px] for the outer boundary
         px-14 for the content alignment
      */}
      <div className="max-w-[1560px] mx-auto w-full h-full flex flex-col px-6 md:px-14">
        ...
        <header className="flex justify-between items-end mb-10 flex-shrink-0">
          <div>
            ...
            <h2 className="text-[48px] font-serif text-[#33302E] leading-none tracking-tighter">
              Signature <span className="italic font-normal">Collection</span>
            </h2>
          </div>
          <button className="group flex items-center gap-3 text-[10px] uppercase tracking-[0.3em] font-bold border-b border-black/10 pb-2 hover:border-[#B59461] transition-all">
            Explore anthology <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
          </button>
        </header>

        {/* --- PROPORTIONAL GRID --- */}
        <div className="flex-grow grid grid-cols-12 grid-rows-6 gap-6 h-full min-h-0">
          
          {/* Featured Vertical - Left */}
          <div className="col-span-12 md:col-span-3 row-span-6 relative overflow-hidden group shadow-sm rounded-sm">
            ...
            <div className="absolute bottom-8 left-8 text-white z-10">
              <h3 className="text-xl font-serif italic mb-1 drop-shadow-md">{items[2].name}</h3>
              <p className="text-[9px] tracking-[0.4em] uppercase font-medium text-white/90">Heritage Edition</p>
            </div>
          </div>

          {/* Top Center Wide (Hero Image) */}
          <div className="col-span-12 md:col-span-6 row-span-3 relative overflow-hidden group shadow-sm rounded-sm">
          ...
          {/* Top Right Square */}
          <div className="col-span-12 md:col-span-3 row-span-2 relative overflow-hidden bg-white flex items-center justify-center p-8 rounded-sm shadow-sm">
            <img src={items[0].img} className="w-full h-full object-contain mix-blend-multiply" alt="" />
            <p className="absolute bottom-4 text-[10px] font-serif italic text-[#33302E] opacity-60">{items[0].name}</p>
          </div>

          {/* Middle Right (Minimalist Text Card) */}
          <div className="col-span-12 md:col-span-3 row-span-2 bg-[#1A1A1A] p-8 flex flex-col justify-center items-center text-center rounded-sm">
            <span className="text-[#B59461] text-[9px] uppercase tracking-[0.5em] mb-4">Craftsmanship</span>
            <h4 className="text-white font-serif text-2xl italic leading-tight">Objects of <br/> Desire</h4>
            <div className="w-10 h-[1px] bg-[#B59461]/40 mt-6"></div>
          </div>

          {/* Bottom Row Mosaic */}
          <div className="col-span-12 md:col-span-3 row-span-3 relative overflow-hidden group rounded-sm">
            <img src={items[4].img} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" alt="" />
          </div>
          ...
          <div className="col-span-12 md:col-span-3 row-span-2 relative overflow-hidden shadow-sm rounded-sm">
